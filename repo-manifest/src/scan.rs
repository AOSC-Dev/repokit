...
pub fn calculate_decompressed_size<R: Read + Seek>(mut reader: R, archive: &Path) -> Result<u64> {
    ...
    let size = if buffer == b"hsqs"[..] {
        ...
        let tmp_path = tmp_path
            .to_str()
            .ok_or_else(|| anyhow!("Cannot to str!"))?
            .replace("'", "'\"'\"'");
        let archive = archive
            .to_str()
            .ok_or_else(|| anyhow!("Invalid archive path"))?
            .replace("'", "'\"'\"'");
        let command = Command::new("unsquashfs")
            .arg("-f")
            .arg("-d")
            .arg(&tmp_path)
            .arg(archive)
        ...
        if !command.status.success() {
            return Err(anyhow!(
                "Cannot run unsquashfs! Err: \n{}\n{}",
                String::from_utf8_lossy(&command.stderr),
                String::from_utf8_lossy(&command.stdout)
            ));
        }
        let command = Command::new("du").arg("-s").arg(&tmp_path).output()?;
        if !command.status.success() {
            return Err(anyhow!(
                "Cannot run du -s ! Err: \n{}\n{}",
                String::from_utf8_lossy(&command.stderr),
        ...
        let split_size = std::str::from_utf8(&command.stdout)?
            .split_whitespace()
            .collect::<Vec<_>>()[0];
        let size: u64 = split_size.parse()?;
        let size = size * 1024;
